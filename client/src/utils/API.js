...
export const deleteApiary = (_id, token) => {
  return fetch(`/api/users/apiary/${apiary._id}`, {
    method: "DELETE",
    headers: {
      authorization: `Bearer ${token}`,
    },
...
export const searchGooglemaps = (query) => {
  return fetch(
    `https://maps.googleapis.com/maps/api/js?key=YOUR_API_KEY&callback=initMaps?q=${query}`
  );
};
